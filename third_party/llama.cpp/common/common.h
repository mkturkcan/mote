...
struct common_params_speculative_draft {
    ...
    int32_t n_tree_k   = 1;     // tree-draft: max hypotheses per slot (1 = linear, no branching). MTP-only.
    float   tree_p_max = 0.9f;  // branch when the draft top-1 prob is below this (an uncertain step)
