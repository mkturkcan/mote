...
struct block_q3_Kx8 {
    ...
    uint8_t   hmask[QK_K / 8 * 8]; // quants - high bit  (32 bytes/col, blocklen=4 interleave)
    uint8_t   qs[QK_K / 4 * 8];    // quants - low 2 bits (64 bytes/col, blocklen=4 interleave)
    uint8_t   scales[12 * 8];      // scales, 6-bit packed (12 bytes/col). MUST stay packed: the repacked
                                   // block must fit 8*sizeof(block_q3_K)=880B; unpacked (128B) would overflow.
};
static_assert(sizeof(block_q3_Kx8) == sizeof(ggml_half) * 8 + QK_K / 8 * 8 + QK_K / 4 * 8 + 12 * 8, "wrong q3_K block size/padding");
...
void ggml_gemm_q3_K_8x4_q8_K(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc);
